...
function entryPoint(){
	...
	if(args.count() == 1){
		number = Number(args.item(0));
		if(isNaN(number)){
			WScript.Echo("\n\t" + number + "  >>-(means)->  " + toRoman(number));
		} else {
			WScript.Echo("\n\t" + number + "  >>-(means)->  " + toArabic(number));
		}	
	} else showHelp();
}

function showCopy(){
	WScript.Echo("Roman2Arabic Convertor - Copyright 2001 by Vasian CEPA");
}

function showHelp(){
	showCopy();
	var helpStr = "Usage: csript roman.js number\n"
	+ "Converts a roman number to an arabic one, and vice-versa.\n"
	+ "If 'number' is arabic the roman counterpart is returned.\n"
	+ "If 'number' is roman the arabic counterpart is returned.\n"
	+ "Range 1-9999 only\n"
	+ "E.g.:\n"
	+ "\tcsript //NoLogo roman.js 33\n\t ->returns XXXIII\n"
	+ "\tcsript //NoLogo roman.js DCLXVI\n\t ->returns 666\n"
	+ "***********************************************";
